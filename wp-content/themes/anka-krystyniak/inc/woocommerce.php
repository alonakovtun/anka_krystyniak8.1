...
add_filter('woocommerce_available_payment_gateways', 'ak_payment_method_by_shipping');
function ak_payment_method_by_shipping($gateways)
{
    ...
    $chosen_shipping_method = isset(WC()->session) && is_array(WC()->session->get('chosen_shipping_methods')) ? WC()->session->get('chosen_shipping_methods')[0] : '';

    // check the shipping method
    if ('flat_rate:7' === $chosen_shipping_method) {
        if (isset($gateways['cod'])) {
            // only return cod payment method for cod shipping
            return array_filter($gateways, function ($k) {
                return $k == 'cod';
...
add_filter('woocommerce_package_rates', 'hide_cod_shipping_for_big_orders', 10, 2);
function hide_cod_shipping_for_big_orders($rates)
{
    if (isset($rates['flat_rate:7']) &&  WC()->cart->get_cart_contents_total() > 500 && strpos(WC()->cart->get_cart_total(), 'EUR') === false) {
        // remove cod shipping option
        unset($rates['flat_rate:7']);

        // remove regular shipping option if free is available
