...
?>
	...
	public function calculate_shipping( $package=array() ) {
		...
		foreach( $packages as $package ) {
			...
			if ( $package_requests ) {
				...
				$rate_response 		= $this->process_result( $this->get_result($rate_requests),'',$rate_requests );
				...
				if( isset($this->custom_services[96]['enabled']) && $this->custom_services[96]['enabled'] ) {
					...
					$rates[96][]	= $this->process_result( $this->get_result($rate_requests),'',$rate_requests );
				}
				...
				if( isset($this->settings['services']['US48']['enabled']) && $this->settings['services']['US48']['enabled'] ) {
					...
					if( $this->soap_available ) {
						...
						$rate_requests	= $this->get_rate_requests_gfp( $package_requests, $package);
						$rates['US48'][]	= $this->process_result_gfp( $this->get_result_gfp($rate_requests,'UPS GFP'));
					...
					foreach ($this->freight_services as $service_code => $value) {
						if( ! empty($this->settings['services'][$service_code]['enabled']) ) {
							...
							// foreach( $package_requests as $package_key => $package_request) {
								...
								$freight_rate_response = $this->process_result( $this->get_result($freight_rate_requests, 'freight'), 'json' );
				...
				if($surepost_check!=1 ){
					foreach ( $this->ups_surepost_services as $service_code ) {
						...
						$rate_response			= $this->process_result( $this->get_result($rate_requests, 'surepost'),'',$rate_requests );
	...
	public function get_result_gfp($request, $request_type='')
	{
		$ups_response = null;
		...
		if( $this->debug ) {
			$this->debug( "UPS GFP REQUEST: <pre>" . print_r( htmlspecialchars($client->__getLastRequest()) , true ) . '</pre>' );
			$this->debug( "UPS GFP RESPONSE: <pre>" . print_r( htmlspecialchars($client->__getLastResponse()) , true ) . '</pre>' );

			$this->diagnostic_report( '------------------------ UPS GFP REQUEST ------------------------' );
			$this->diagnostic_report( htmlspecialchars($client->__getLastRequest()) );
			$this->diagnostic_report( '------------------------ UPS GFP RESPONSE ------------------------' );
