...
<?php
		...
		foreach ( $created_shipments_details_array as $shipmentId => $created_shipments_details ){
			...
			if( !empty($ups_label_details_array[$shipmentId]) ) {
				...
				foreach ( $ups_label_details_array[$shipmentId] as $ups_label_details ) {
					...
					if( $this->dangerous_goods_manifest && $ups_dangerous_goods_manifest_required && !empty($ups_dangerous_goods_manifest_data) && is_array($ups_dangerous_goods_manifest_data) ) {

						echo '<a class="button button-primary tips" target="_blank" href="'.admin_url( '/?ph_ups_dgm='.base64_encode($post->ID)).'" data-tip="'.__('Downloads UPS Dangerous Goods Manifest', 'ups-woocommerce-shipping').'">'.__('Dangerous Goods Manifest', 'ups-woocommerce-shipping').'</a><br><br>';
					}
	...
	*/
	...
	function wf_ups_shipment_confirmrequest_GFP( $order,$shipment=array() ) {
		...
		$billing_address_preference = $this->get_product_address_preference( $order, false, $ups_settings );
		...
		if( is_array($shipment) )
		{
				...
				$xml_request .= '<CustomerContext>'.$order->get_id().'</CustomerContext>';
	...
	function wf_ups_shipment_confirmrequest( $order,$return_label=false ) {
		...
		$billing_address_preference = $this->get_product_address_preference( $order, $return_label, $ups_settings );
				...
				if( in_array($shipping_service,array_keys($this->freight_services)) || in_array($shipment['shipping_service'],array_keys($this->freight_services)) )
				{
					...
					$request_arr['Shipment']=array();
						...
						if( $this->commercial_invoice && ( $from_address['country'] != $to_address['country'] ) && !(in_array($from_address['country'], $this->eu_array) && in_array($to_address['country'], $this->eu_array) ) ){ // Commercial Invoice is available only for international shipments
							...
							if( !empty($this->terms_of_shipment) ) {
								$shipmentServiceOptions['InternationalForms']['TermsOfShipment']	=	$this->terms_of_shipment;
							}
						...
						if( is_a( $product_data, 'WC_Product' ) && $product_data->needs_shipping() ) {
							...
							$product_details = array(
								...
								'Unit'			=>	array(
									'Number'	=>	$product_quantity,
									'UnitOfMeasurement'	=>	array('Code'	=>	'EA'), // EA = Each
									'Value'		=>	round(($product_data->get_price() / $this->wcsups->conversion_rate),2),
								),
								'OriginCountryCode'	=>	$countryofmanufacture,
								'NumberOfPackagesPerCommodity'	=>	'1',
								'ProductWeight'	=>	array(
	...
	function ph_ups_print_dangerous_goods_manifest() {
		...
		$order_id		= base64_decode($_GET['ph_ups_dgm']);
...
function ups_accept_shipment($order_id){
	...
	public function ph_create_dangerous_goods_manifest( $order_id ) {
		...
		if( !empty($hazmat_products) ) {
			...
			update_post_meta( $order_id, 'ph_ups_dangerous_goods_manifest_data', $hazmat_products );
		}
	...
	*/
	...
	public function get_product_address_preference( $order, $return_label = false, $ups_settings ) {
	...
	// Automatic Package Generation
	public function ph_ups_auto_generate_packages( $order_id, $minute = '', $ups_settings) {
	...
	// Automatic Label Generation
	public function ph_ups_auto_create_shipment( $order_id, $minute = '', $ups_settings, $weight_arr, $length_arr, $width_arr, $height_arr, $service_arr, $insurance ) {
