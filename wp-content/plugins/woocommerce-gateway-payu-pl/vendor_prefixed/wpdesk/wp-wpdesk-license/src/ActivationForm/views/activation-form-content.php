...
if ($is_active) {
    if ($update_possible) {
        ...
        \esc_html_e(\__('Plugin\'s API key has been successfully activated, but your subscription has expired. In order to receive the upcoming updates and support please renew your subscription.', 'wp-wpdesk-license'));
    }
} else {
...
if ($is_active) {
    ...
    if (!$update_possible) {
        ...
        echo \esc_html(\__('Renew subscription →', 'wp-wpdesk-license'));
        ?></a>
			<?php 
    }
    ?>
