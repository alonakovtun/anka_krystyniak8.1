...
namespace WGPayuVendor\WPDesk\Tracker;

use WGPayuVendor\WPDesk\PluginBuilder\Plugin\Hookable;
use WGPayuVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use WGPayuVendor\WPDesk\View\Resolver\DirResolver;
class OptInPage implements \WGPayuVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function wpdesk_tracker_page()
    {
        ...
        $shop = new \WGPayuVendor\WPDesk\Tracker\Shop($shop_url);
        $terms_url = $shop->get_usage_tracking_page();
        $logo = $shop->get_shop_logo_file();
        $logo_url = \plugin_dir_url(__FILE__) . '../../../assets/images/' . $logo;
        $renderer = new \WGPayuVendor\WPDesk\View\Renderer\SimplePhpRenderer(new \WGPayuVendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
        // WPCS: XSS ok.
    ...
    public function admin_init()
    {
        if (isset($_GET['wpdesk_tracker']) && $_GET['wpdesk_tracker'] === $this->plugin_slug) {
            if (isset($_GET['allow']) && isset($_GET['security']) && \wp_verify_nonce($_GET['security'], $this->plugin_slug)) {
                $persistence = new \WGPayuVendor\WPDesk_Tracker_Persistence_Consent();
